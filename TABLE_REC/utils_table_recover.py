...
def find_missing_cells(logic_points):
    all_rows = []
    all_cols = []
    
    for r_start, r_end, c_start, c_end in logic_points:
        all_rows.extend([r_start, r_end])
        all_cols.extend([c_start, c_end])
    ...
    for r in range(max_row + 1):
        for c in range(max_col + 1):
            if not grid[r][c]:
                missing_cells.append((r, r, c, c))
    
    return missing_cells, max_row, max_col

def get_idx2point(polygons, logic_points, max_row, max_col):
    start_row_points = [[] for i in range(max_row + 2)]
    start_col_points = [[] for i in range(max_col + 2)]
    end_row_points = [[] for i in range(max_row + 2)]
    end_col_points = [[] for i in range(max_col + 2)]

    for polygon, logic_point in zip(polygons, logic_points):
        x1, y1 = polygon[0]
        x2, y2 = polygon[2]
        start_row, end_row, start_col, end_col = logic_point

        start_row_points[start_row].append(y1)
        start_col_points[start_col].append(x1)

        end_row_points[end_row].append(y2)
        end_col_points[end_col].append(x2)
    
        start_row_points[end_row + 1].append(y2)
        start_col_points[end_col + 1].append(x2)

        end_row_points[start_row - 1].append(y1)
        end_col_points[start_col - 1].append(x1)


    return start_row_points, start_col_points, end_row_points, end_col_points



def fill_missing_cells(polygons, logic_points):
    missing_cells, max_row, max_col = find_missing_cells(logic_points)
    
    if len(missing_cells) == 0:
        return polygons, logic_points

    try:
        new_polygons = np.array(polygons).tolist()
        new_logic_points = np.array(logic_points).tolist()
        start_row_points, start_col_points, end_row_points, end_col_points = get_idx2point(new_polygons, new_logic_points, max_row, max_col)

        for start_row, end_row, start_col, end_col in missing_cells:
            print(start_row, end_row, start_col, end_col)
            x1 = sum(start_col_points[start_col]) / len(start_col_points[start_col])
            y1 = sum(start_row_points[start_row]) / len(start_row_points[start_row])
            x2 = sum(end_col_points[end_col]) / len(end_col_points[end_col])
            y2 = sum(end_row_points[end_row]) / len(end_row_points[end_row])
            ...
            new_polygons.append(miss_polygon)
            new_logic_points.append([start_row, end_row, start_col, end_col])
            # print(f'===== >> new_polygons and new_logic_points are {new_polygons}, {new_logic_points}')
        new_polygons = np.array(new_polygons)
        new_logic_points = np.array(new_logic_points)
        return new_polygons, new_logic_points
    except:
        return polygons, logic_points
...
def is_box_contained(inner, outer):
    return (
        inner[0] >= outer[0] and  # xmin
        inner[1] >= outer[1] and  # ymin
        inner[2] <= outer[2] and  # xmax
        inner[3] <= outer[3]      # ymax
    )


def filter_polygons(polygons, iou_threshold=0.5):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if iou >= iou_threshold:
                area_i = (boxes[i][3] - boxes[i][1]) * (boxes[i][2] - boxes[i][0])
                area_j = (boxes[j][3] - boxes[j][1]) * (boxes[j][2] - boxes[j][0])
                if area_i >= area_j:
                    keep[j] = False
                else:
                    keep[i] = False
...
def remove_fully_contained_boxes(polygons, logic_points, is_vis = False):

    N = logic_points.shape[0]
    keep = np.ones(N, dtype=bool)
    ...
    for i in range(N):
        for j in range(N):
            if i == j:
                continue
            if (logic_points[j][0] <= logic_points[i][0] and
                logic_points[j][1] >= logic_points[i][1] and
                logic_points[j][2] <= logic_points[i][2] and
                logic_points[j][3] >= logic_points[i][3]):
                keep[i] = False
                break
    if is_vis:
        for idx, k in enumerate(keep):
            if not k:
                print(f'===delete contained boxesd ', logic_points[k])
    return polygons[keep], logic_points[keep]
